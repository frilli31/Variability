...
def compute_edit_distance_variability(log: lg.EventLog) -> float:
    ...
    Returns:
        float: a number between 0 and 1 included. The higher the number the
            ...
            - 0 : if all traces has nothing in common one another
            - 1 : all traces belongs to the same variant (have the same
                  sequence of events)
    """
...
'''
def compute_my_variability_old(log: lg.EventLog):
    """Compute the sum of edit distances (Levenshtein distance) between each
    pairs of traces divided by the sum of the maximum edit distances between
    each pair of traces.
    ...
    Returns:
        float: a number between 0 and 1 included.
    """
    variants_and_counts = Counter(
        tuple(event["concept:name"] for event in case) for case in log
    )
    ...
    for (variant1, num_of_items_1), (variant2, num_of_items_2) in combinations(
        ...
        sum_of_distances += (
                num_of_items_1 * num_of_items_2 * editdistance.eval(variant1, variant2) * 2
        )
        sum_of_max_distances += (
                num_of_items_1 * num_of_items_2 * max(len(variant1), len(variant2)) * 2
        )

    # Here we add to max distances the case of picking the traces that belong
    # to the same variant, so we its length times the number of times it can
    # occur
    sum_of_max_distances += sum(
        len(k) * v * (v - 1) for k, v in variants_and_counts.items()
    )
